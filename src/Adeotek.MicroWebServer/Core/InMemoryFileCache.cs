...
namespace Adeotek.MicroWebServer.Core
{
    ...
    public class InMemoryFileCache
    {
        public delegate bool InsertHandler(InMemoryFileCache cache, string key, byte[] value, TimeSpan timeout);
        ...
        {
            handler ??= (InMemoryFileCache cache, string key, byte[] value, TimeSpan timespan) => cache.Add(key, value, timespan);
